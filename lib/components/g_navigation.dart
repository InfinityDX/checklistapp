...
class GNavigation {
  ...
  static Future<bool> pop<T>({BuildContext? context, T? arg}) {
    var prevRoute = ModalRoute.of(context ?? _context)?.settings.name ?? '';
    log('Pop on $prevRoute', name: 'AppNavigation');
    return Navigator.of(context ?? _context).maybePop(arg);
  }
